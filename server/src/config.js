...
export function createConfig() {
  ...
  return {
    ...
    rag: {
      enabled: normalizeBooleanEnv(process.env.AGENT_RAG_ENABLED, Boolean(String(process.env.AGENT_RAG_DATABASE_URL || '').trim())),
      databaseUrl: String(process.env.AGENT_RAG_DATABASE_URL || '').trim(),
      embeddingDimension: readNumberEnv('AGENT_RAG_EMBEDDING_DIM', 1024),
      embeddingBaseURL: normalizeBaseUrl(process.env.AGENT_RAG_EMBEDDING_BASE_URL),
      embeddingApiKey: String(process.env.AGENT_RAG_EMBEDDING_API_KEY || '').trim(),
      embeddingModel: String(process.env.AGENT_RAG_EMBEDDING_MODEL || '').trim(),
      embeddingProvider: String(process.env.AGENT_RAG_EMBEDDING_PROVIDER || 'auto').trim() || 'auto',
      embeddingTimeoutMs: readNumberEnv('AGENT_RAG_EMBEDDING_TIMEOUT_MS', 30000),
      maxSearchResults: readNumberEnv('AGENT_RAG_MAX_SEARCH_RESULTS', 6),
      chunkMaxChars: readNumberEnv('AGENT_RAG_CHUNK_MAX_CHARS', 1200),
      chunkOverlapChars: readNumberEnv('AGENT_RAG_CHUNK_OVERLAP_CHARS', 160)
    },
    workspace: {
